...
def plot_world(agent, rays_endpoints=None):
    ...
    tri = np.array([  # 삼각형 좌표 계산
        ...
        [x + np.cos(th + 2.5) * L / 1.5, y + np.sin(th + 2.5) * L / 1.5],  # 왼쪽 뒤. 본체 중심에서 방향을 왼쪽으로 2.5rad (~143°) 돌린 방향
        [x + np.cos(th - 2.5) * L / 1.5, y + np.sin(th - 2.5) * L / 1.5],  # 오른쪽 뒤. 본체 중심에서 오른쪽으로 2.5rad (~143°) 돌린 방향
    ])
...
if __name__ == "__main__":
    ...
    plot_world(agent, rays_endpoints=endpoints)