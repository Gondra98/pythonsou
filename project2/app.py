...
def api_battery_predict():
    """좌표 기반 비행거리, 풍속, 고도, 총회전량을 입력받아 잔여 배터리를 예측."""
    ...
    if distance <= 0 or distance > 1000:
        return jsonify({"success": False, "message": "비행 거리는 0보다 크고 1000m 이하이어야 합니다."}), 400

    # 그래프 표시용: 0m부터 최종 거리까지 구간별 잔여 배터리 예측
    distance_values = np.linspace(0.1, distance, 20)

    model_input = pd.DataFrame({
        "풍속(m/s)": [wind] * len(distance_values),
        "비행고도(m)": [altitude] * len(distance_values),
        "2D이동거리(m)": distance_values,
        "총회전량(deg)": [rotation] * len(distance_values)
    })
    ...
    return jsonify({
        ...
        "graph": {
            "distance": [round(float(x), 2) for x in distance_values],
