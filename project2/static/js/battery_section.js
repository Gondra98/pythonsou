...
function bpInitMap() {
    ...
    bpMap = new ol.Map({
        target: 'battery-vmap',
        layers: [baseLayer, lineLayer, markerLayer],
        view: new ol.View({
...
function bpUpdateDistanceDisplay() {
    ...
    if (bpCoords.length < 2) {
        result.innerHTML = `
            <div>입력 좌표 수: <strong>${bpCoords.length}개</strong> / 최대 4개</div>
            <div>비행 거리: <strong>좌표를 최소 2개 이상 입력하면 계산됩니다.</strong></div>
        `;
        return;
    }
...
function bpValidateInputs() {
    ...
    const distance = bpGetTotalDistanceMeters();
    const rotation = bpGetTotalRotationDegrees();
    ...
    if (distance > BP_LIMITS.maxDistance) {
        alert('비행 거리는 최대 1000m 이하로 입력해야 합니다.');
        return null;
    }
